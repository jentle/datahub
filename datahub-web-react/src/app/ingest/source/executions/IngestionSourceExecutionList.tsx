...
export const IngestionSourceExecutionList = ({ urn, isExpanded, lastRefresh, onRefresh }: Props) => {
    ...
    const onCancelExecutionRequest = (executionUrn: string) => {
        cancelExecutionRequestMutation({
            ...
            .finally(() => {
                message.success({
                    content: t(`Successfully submitted cancellation request!`),
                    duration: 3,
                });
        ...
        Modal.confirm({
            ...
            content: (
                <div>
                    ...
                    <br />
                    <br /> {t('Are you sure you want to continue?')}
                </div>
            ),
            onOk() {
                ...
                rollbackIngestion({ variables: { input: { runId } } })
                    .then(() => {
                        setTimeout(() => {
                            ...
                            refetch();
                            onRefresh();
                            message.success(t('Successfully requested ingestion rollback'));
                        }, 2000);
                    })
                    .catch(() => {
                        message.error(t('Error requesting ingestion rollback'));
                    });
            },
            onCancel() {},
            okText: t('Rollback'),
    ...
    return (
        <ListContainer>
            {!data && loading && <Message type="loading" content={t('Loading executions...')} />}
            {error && (
                <Message
                    type="error"
                    content={t('Failed to load ingestion executions! An unexpected error occurred.')}
                />
            )}
