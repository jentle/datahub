...
export default function IngestedAssets({ id }: Props) {
    ...
    return (
        <>
            ...
            <HeaderContainer>
                <TitleContainer>
                    <Typography.Title level={5}>{t('Ingested Assets')}</Typography.Title>
                    {(loading && <Typography.Text type="secondary">{t('Loading...')}</Typography.Text>) || (
                        <>
                            {(total > 0 && (
                                <Typography.Paragraph type="secondary">
                                    {t('The following asset types were ingested during this run.')}
                                </Typography.Paragraph>
                            )) || <Typography.Text>{t('No assets were ingested.')}</Typography.Text>}
                        </>
                    )}
                </TitleContainer>
                {!loading && (
                    <TotalContainer>
                        ...
                        <TotalText style={{ fontSize: 16, color: ANTD_GRAY[8] }}>
                            <b>{formatNumber(total)}</b> {t('assets')}
                        </TotalText>
                    </TotalContainer>
                )}
            </HeaderContainer>
            <EntityCountsContainer>
                {countsByEntityType.map((entityCount) => (
                    <EntityCount>
                        ...
                        <Typography.Text type="secondary">{t(entityCount.displayName)}</Typography.Text>
                    </EntityCount>
                ))}
            </EntityCountsContainer>
