...
export const ExecutionDetailsModal = ({ urn, visible, onClose }: Props) => {
    ...
    return (
        <Modal
            ...
            title={
                <HeaderSection>
                    <StyledTitle level={4}>{t('Ingestion Run Details')}</StyledTitle>
                </HeaderSection>
            }
            visible={visible}
            onCancel={onClose}
        >
            {!data && loading && <Message type="loading" content={t('Loading execution details...')} />}
            {error && message.error(t('Failed to load execution details :('))}
            <Section>
                <StatusSection>
                ...
                <LogsSection>
                    <SectionHeader level={5}>{t('Logs')}</SectionHeader>
                    <SectionSubHeader>
                        <SubHeaderParagraph type="secondary">
                            {t('View logs that were collected during the ingestion run.')}
                        </SubHeaderParagraph>
                    ...
                    <Typography.Paragraph ellipsis>
                        ...
                        {isOutputExpandable && (
                            <ShowMoreButton type="link" onClick={() => setShowExpandedLogs(!showExpandedLogs)}>
                                {t(showExpandedLogs ? 'Hide' : 'Show More')}
                            </ShowMoreButton>
                        )}
