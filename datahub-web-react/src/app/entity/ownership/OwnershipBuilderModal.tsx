...
export const OwnershipBuilderModal = ({ isOpen, onClose, refetch, ownershipType }: Props) => {
        ...
        if (ownershipType) {
            updateOwnershipTypeMutation({
                ...
                .catch((e: unknown) => {
                    ...
                    if (e instanceof Error) {
                        message.error({
                            content: t(`Failed to update ownership type`),
                            duration: 3,
                        });
                    }
                });
    ...
    return (
        <Modal
            ...
            title={
                <TitleContainer>
                    <TitleText>{t(titleText)}</TitleText>
                </TitleContainer>
            }
            footer={null}
        >
            <Form form={form}>
                <FormItemContainer>
                    <FormItemTitle>{t('Name')}</FormItemTitle>
                    <StyledFormItem
                        name="name"
                        rules={[
                            {
                                required: true,
                                message: t('Please input a name for the ownership type'),
                            },
                            { whitespace: true },
                            { min: 1, max: 50 },
                        ]}
                    >
                        <Input
                            ...
                            placeholder={t('Ownership type name')}
                            onChange={(e) => {
