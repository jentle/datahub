...
export default function QueryBuilderModal({ initialState, datasetUrn, onClose, onSubmit }: Props) {
    ...
    const confirmClose = () => {
        Modal.confirm({
            title: t(`Exit Query Editor`),
            content: t(`Are you sure you want to exit the editor? Any unsaved changes will be lost.`),
            onOk() {
                setBuilderState(DEFAULT_STATE);
                onClose?.();
            },
    ...
    return (
        <ClickOutside onClickOutside={confirmClose} wrapperClassName="query-builder-modal">
            <StyledModal
                ...
                title={<Typography.Text>{t(`${isUpdating ? 'Edit' : 'New'} Query`)}</Typography.Text>}
