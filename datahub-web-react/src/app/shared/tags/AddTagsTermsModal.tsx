...
export default function EditTagTermsModal({
    ...
    return (
        <Modal
            title={`${t(operationType === OperationType.ADD ? 'Add' : 'Remove')} ${entityRegistry.getEntityName(type)}`}
            visible={visible}
        ...
        >
            <ClickOutside onClickOutside={() => setIsFocusedOnInput(false)}>
                <TagSelect
                    ...
                    ref={inputEl}
                    filterOption={false}
                    placeholder={`${t('Search for...')} ${entityRegistry.getEntityName(type)?.toLowerCase()}`}
                    showSearch
