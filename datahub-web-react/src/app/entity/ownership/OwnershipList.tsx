...
export const OwnershipList = () => {
    ...
    return (
        <>
            {!data && loading && <Message type="loading" content={t('Loading Ownership Types...')} />}
            {error &&
                message.error({
                    content: t(`Failed to load Ownership Types! An unexpected error occurred.`),
                    duration: 3,
                })}
            <TabToolbar>
                <Button type="text" onClick={onClickCreateOwnershipType}>
                    <PlusOutlined /> {t('Create new Ownership Type')}
                </Button>
                <SearchBar
                    initialQuery=""
                    placeholderText={t('Search by Name...')}
                    suggestions={[]}
