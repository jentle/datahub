...
export const IngestionSourceList = () => {
    ...
    const executeIngestionSource = (urn: string) => {
        createExecutionRequestMutation({
            ...
            .then(() => {
                ...
                message.success({
                    content: t(`Successfully submitted ingestion execution request!`),
                    duration: 3,
                });
        ...
        if (focusSourceUrn) {
            ...
            updateIngestionSource({ variables: { urn: focusSourceUrn as string, input } })
                .then(() => {
                    ...
                    message.success({
                        content: t(`Successfully updated ingestion source!`),
                        duration: 3,
                    });
    ...
    const onExecute = (urn: string) => {
        Modal.confirm({
            title: t(`Confirm Source Execution`),
            content: t("Click 'Execute' to run this ingestion source."),
            onOk() {
    ...
    const onDelete = (urn: string) => {
        Modal.confirm({
            title: t(`Confirm Ingestion Source Removal`),
            content: t(
    ...
    return (
        <>
            {!data && loading && <Message type="loading" content={t('Loading ingestion sources...')} />}
            {error && (
                <Message type="error" content={t('Failed to load ingestion sources! An unexpected error occurred.')} />
            )}
            <SourceContainer>
                <TabToolbar>
                    <div>
